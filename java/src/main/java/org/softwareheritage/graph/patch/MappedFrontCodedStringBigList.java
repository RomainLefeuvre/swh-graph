...
package org.softwareheritage.graph.patch;
...
public class MappedFrontCodedStringBigList extends AbstractObjectBigList<MutableString> implements RandomAccess, Closeable {
     ...
     */
    protected ByteMappedBigList byteList;
    /**
    ...
    protected MappedFrontCodedStringBigList(MappedFrontCodedStringBigList source) {
        this.n = source.n;
        this.ratio = source.ratio;
        this.fileChannel = source.fileChannel;
        this.byteList = source.byteList;
        this.pointers = source.pointers;
    }
    ...
    /**
     * Return a copy of the current MappedFrontCodedStringBigList, by calling ByteMappedBigList.copy() for the byte list
     *
     * @return a copy of the current MappedFrontCodedStringBigList that can be used for multithread computation
     */
    public MappedFrontCodedStringBigList copy() {
        MappedFrontCodedStringBigList copy = new MappedFrontCodedStringBigList(this);
        copy.byteList = this.byteList.copy();
        return copy;
    }
    ...
    private int extract(final long index, final byte a[], final int offset, final int length) {
        ...
        if (currLen < length)
            array.getElements(pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
